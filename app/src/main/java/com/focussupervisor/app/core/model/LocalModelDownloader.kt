 ...
 * 一次下载需要的全部信息。
 *
 * 抽成通用契约，是因为要下的东西不止一个：向量模型现在走它，将来如果识图也要
 * 下模型（备选 B 的微型 VLM），会走同一条。它们的下载逻辑**完全一样**
 * （断点续传、进度上报、SHA-256 校验、临时文件改名），只有「下什么」不同。
 * 为每一类复制一份下载器，等于把那四件事各写一遍 —— 而其中任何一处写漏了
 * 都不会报错，只会表现为「某个模型的下载偶尔是坏的」。
 *
 * @param fileName 落盘文件名。带扩展名，因为它同时是「临时文件叫什么」的依据。
 * @param sha256 期望摘要。**留空表示跳过校验**。当前目录里的两个模型都带摘要，
 *        所以这条路径实际上不会走到；留着是为了让「校验」在契约上是可选的，
 *        而不是散落在若干处 if 里。
 */
